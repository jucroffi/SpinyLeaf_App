...
MASTER_BIB = {
    ...
    ("jordan", "2023"): "Jordan, M. (2023). The power of connection: Self-care strategies of social wellbeing. *Journal of Interprofessional Education & Practice, 31*, 100586. https://doi.org/10.1016/j.xjep.2022.100586",
    ("kearns", "2015"): "Kearns, A., Whitley, E., Tannahill, C., & Ellaway, A. (2015). Loneliness, social relations and health and wellbeing in deprived communities. *Psychology, Health & Medicine, 20*(3), 332–344. https://doi.org/10.1080/13548506.2014.940354",
    ...
    ("williams", "2005"): "Williams, J. (2005). Designing neighbourhoods for social interaction: The case of cohousing. *Journal of Urban Design, 10*(2), 195–227. https://doi.org/10.1080/13574800500086998",
    ...
    ("nguyen", "2024"): "Nguyen, G. T. N., Tsaih, L. S.-J., Chen, J. C.-P., Tamariska, S. R., Coelho, A. M. F., & Kung, H.-Y. (2024). Balcony usage as a space to achieve human well-being during pandemic COVID-19. *Journal of Asian Architecture and Building Engineering*, 1–13. https://doi.org/10.1080/13467581.2024.2370408",
    ...
    ("song", "2024"): "Song, T., Xu, L., Zhao, F., & Du, Y. (2024). Healing properties of residential balcony: Assessment of the characteristics of balcony space in Shanghai’s collective housing. *Journal of Building Engineering, 87*, 108992. https://doi.org/10.1016/j.jobe.2024.108992",
    ("yeom", "2020"): "Yeom, S., Kim, H., Hong, T., Park, H. S., & Lee, D.-E. (2020). An integrated psychological score for occupants based on their perception and emotional response according to the windows’ outdoor view size. *Building and Environment, 180*, 107019. https://doi.org/10.1016/j.buildenv.2020.107019",
}
...
def _norm_key_from_citation(cite_text: str):
    ...
    try:
        import unicodedata
        last = ''.join(c for c in unicodedata.normalize('NFKD', last) if not unicodedata.combining(c))
    except Exception:
        pass
    
    last = last.replace("weijs-perrée", "weijs-perree").replace("weijs-perrée", "weijs-perree").replace("smektała", "smektala")
    return (last, year)
...
def get_completion(prompt, model="gpt-4", temperature=0.3, max_tokens=1024):
...
thermal_report = get_completion(thermal_text)
...
daylight_report = get_completion(daylight_text)
...
acoustic_report = get_completion(acoustic_text)
...
airquali_report = get_completion(airquali_text)

comfort_report = comfort_intro + thermal_report + daylight_report + airquali_report + acoustic_report
...
delight_report = get_completion(delight_text)
...
social_report = get_completion(social_text)
...
def add_report_section(doc, title, report_text, dimension, image_dir):
    """
    Note: Ignores any '## References' the model prints. We build the final References
